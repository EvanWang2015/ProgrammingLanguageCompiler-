/* I have neither given nor received any unauthorized aid on this
 assignment_ Haitang Wang*/ 

//The input format exactly fowllows the requirement declaimed by prof. Mishra.
...
main () 
{
  ...
  do {
      ...
      read();
      //result_print(k);
      result_save(k, outFile);
      k++;
      cout << "\n";
  } while( (INM[0].size()!=0) || (REB[0].size()!=0) || (ADB.size()!=0) || (AIB.size()!=0) || (LIB.size()!=0) || (INB.size()!=0));
...
void inatilize_read(string *INMP, string *RGFP, string *DAMP) {  // initialization to read data to INM, DAM and RGF;
     string line;
...
void load() {
     ...
     //Load data from memory and save to REB (Result Buffer) when ADB is not empty
     if (ADB.size()!=0)
     {
         string opps[2];
         size_t s1 = ADB.find("<");
...
void result_save(int k, ofstream& outputFile){
     
     //outputFile.open("simulation.txt", fstream::in | fstream::app);
     int i =0;
     outputFile << "STEP " << k << ": ";
     outputFile << "\n";
     outputFile << "INM:  ";
     for (i=0; i<8; i++) {
         if (i <7 && INM[i+1].size()!=0 )
         outputFile << INM[i]<< ",";
         else 
         outputFile << INM[i] << " ";
     }
